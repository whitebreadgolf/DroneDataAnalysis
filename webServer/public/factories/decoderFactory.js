...
.factory('Decoder', function (ngProgressFactory, Notification, $interval){
	...
	return {
		isRunningOrInQueue: function(_flightId){
		...
		stopDecoding: function(_flightId){

			if(currentDecoding.flight_id === _flightId){
				// first check if the decoding was finished
				if(currentDecoding.finished === true){
					// send string to backend service

					// END sending string

					Notification({message: 'flight '+_flightId+ ' completed'}, 'success');
				}
				else Notification({message: 'flight '+_flightId+ ' stopped'}, 'info');
				...
				if(decodeQueue.length > 0){ 
					...
					this.startDecoderHelper(decodingProcess.flightId, decodingProcess.file);
				}
		...
		startDecoder: function(_flightId, _file){

			// start decoder now
			if(!currentDecoding) this.startDecoderHelper(_flightId, _file);
		...
		startDecoderHelper: function(_flightId, _file){
			...
			// start progress bar
			currentDecoding.progress_bar.setHeight('5px');
		        ...
		        if (segments.length > 10) {
		            ...
		            for (j = 1; j < segments.length - 1; j++) {
		                if (segments[j].substring(254, 256) == '55') {
		                    ...
		                    if (currentDecoding.csv_string == '')
		                        currentDecoding.csv_string = "Latitude (Deg), Longitude (Deg), GPS Altitude (m), N Velocity(m/s), E Velocity(m/s), D Velocity(m/s), Velocity(m/s), Ground Speed(m/s), AccelerometerX(g), AccelerometerY(g), AccelerometerZ(g), GyroX(rad/s), GyroY(rad/s), GyroZ(rad/s), Barometric Alt(m), QuaternionX, QuaternionY, QuaternionZ, QuaternionW, Roll(deg), Pitch(deg), Yaw(deg), MagneticX, MagneticY, MagneticZ, Satellites, Sequence(135 Hz) \n";
